document.addEventListener('DOMContentLoaded', () => {
  ...
  $('save-preset').addEventListener('click', ()=>{
    ...
    if (m>0){
      let arr = JSON.parse(localStorage.getItem('timerPresets')||'[]');
      if (!arr.includes(m)){ arr.push(m); localStorage.setItem('timerPresets',JSON.stringify(arr)); loadPresets(); }
    }
  });
  ...
  $('start-pause-button').addEventListener('click',()=>{
    ...
    if (sel==='custom'){
      ...
      if (!isNaN(secs)){ timeLeft=duration=secs; mode='work'; updUI(); }
    }
    startPause();
  });
  ...
  // — STATS STUB —  
  // leave for future chart integration
